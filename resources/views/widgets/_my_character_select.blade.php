            ...
            <div class="row">
                @foreach ($characters as $character)
                    <div class="col-lg-2 col-sm-3 col-6 mb-3 user-character category-all category-{{ $character->character_category_id ?: 0 }} {{ isset($selected) && in_array($character->id, $selected) ? 'category-selected' : '' }} {{ (isset($selected) && in_array($character->id, $selected)) || $character->isAvailable ? '' : 'select-disabled' }}"
                        ...
                        <div class="text-center character-item {{ ((isset($selected) && in_array($character->id, $selected)) || $character->isAvailable) && !$character->is_locked ? '' : 'disabled' }}"
                            @if ((!(isset($selected) && in_array($character->id, $selected)) && !$character->isAvailable) || $character->is_locked) data-toggle="tooltip" title="{{ $character->trade_id ? 'This character is in a trade.' : 'This character has an active design update or is locked.' }}" @endif>
                            <div class="mb-1">
